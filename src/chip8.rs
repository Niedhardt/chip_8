...
impl Chip8 {
    ...
    pub fn insert_rom_data(&mut self, rom: RomDriver) {
        let mut index = 0x200 + 1;
        /*
        for i in (0..rom.rom.len()-1).step_by(2) {
            let opcode = ((rom.rom[i] as u16) << 8 ) | rom.rom[i+1] as u16;
            //self.memory[index] = opcode;
            self.execute_opcodes(opcode);
            //println!("{:04x}", opcode);
            //println!("{}", number);
        }*/
...
/*
    ...
    pub fn op_dxyn(&mut self, x: usize, y: usize, n: usize) {
        println!("Display: ");
        let width = (self.v[x] % 64) as usize;
        let height = (self.v[y] % 32) as usize;
